...
*   **数理的背景**: 高次元の画像データから空間的に局所的な特徴を畳み込みによって抽出し、画像の空間情報の圧縮 (ダウンサンプリング) を経て、最終的に10次元の確信度 (分類) ベクトルへと非線形活性化を通して写像する行列演算とテンソル変形 (テンソル平坦化).
*   **技術的アピール（工夫した点）**:
    *   CNNのスクラッチモデルを実装しました.
...
*   **数理的背景**: ResNetの標準よりも解像度が低い入力画像に対して、畳み込みによる画像の空間情報 (空間解像度) の損失をサイズ計算に基づいて抑えつつ、ImageNetの事前学習で培われた高次元の特徴ベクトルによって画像データを表現する能力を活かし、全結合最終層において学習した、各画像クラスをスコア (ロジット) の優劣によって分離する線形超平面によって、10個の画像クラスに正しく射影・分類すること.
*   **技術的アピール（工夫した点）**:
...
*   **数理的背景**: まず、初期画像から任意タイムステップにおけるノイズを加えられた画像をサンプリングします. 次に、タイムステップが条件づけられたU-Netによるノイズ予測モデルを、加えられたノイズ (教師ラベル) との平均二乗誤差をタイムステップ、初期画像、ノイズについて平均化した関数を損失関数として用いて訓練します. 最後に、訓練された予測ノイズを用いて画像を、ノイズにまみれた画像からノイズを除去することによって生成 (サンプリング) します.
*   **技術的アピール（工夫した点）**:
...
*   **数理的背景**: 因果的にマスク (未来隠蔽) したセルフアテンションをパーツに用いた次トークンの予測モデルと、時系列 (文章) を1トークンだけ先にずらすことによって得られた教師ラベルを用いて、交差エントロピー関数を損失関数とした (予測の正確さを、条件付き確率を用いて評価した) 学習のモデル.
*   **技術的アピール（工夫した点）**:
...
*   **数理的背景**: 高次元空間のデータを非線形に圧縮して本質的な情報 (低次元の多様体) だけを残し、そこから再度高次元のデータを復元するモデル. 訓練において正常データの構造だけを**ボトルネック** (データの特徴の圧縮の最深部) に通過させることで、それ以外の異常データが入力された際に、復元に失敗する (再構成誤差が大きくなる) ことを利用した異常検知.
*   **技術的アピール（工夫した点）**:
